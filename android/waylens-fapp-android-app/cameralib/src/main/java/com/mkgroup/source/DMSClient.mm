...
@implementation DMSClient

- (instancetype)initWithIPv4:(NSString *)ipv4 IPv6:(NSString *)ipv6 Port:(long)port carrier:(NSString*)carrier {
    self = [super initWithIPv4:ipv4 IPv6:ipv6 Port:port carrier:carrier];
    _vendor = 0;
    if (self) {
...
- (void)addFaceWithID:(unsigned long long)faceID name:(NSString*)name {
    ...
    if (_vendor == VENDOR_READSENSE) {
        ...
        if (self.dmsDelegate) {
            [self.dmsDelegate onAddFaceResult:DMSErrNoServer];
        }
    }
}
    ...
    switch (pAck->cmd_code) {
        ...
        case DMS_CMD_ListFaceIds: {
            if(pAck->ret_code == 0) {
                ...
                if (self.dmsDelegate) {
                    [self.dmsDelegate onGetFaceList:arr];
                }
            } else{
        ...
        case RS_CMD_CaptureImage: {
            if(pAck->ret_code == 0) {
                ...
                if (img->has_face == false) {
                    if (self.dmsDelegate) {
                        [self.dmsDelegate onAddFaceResult:DMSErrNoFace];
                    }
                    break;
                } else {
                ...
                if (self.dmsDelegate) {
                    [self.dmsDelegate onAddFaceResult:DMSErrInternal];
                }
            }
        }
        break;
        case RS_CMD_AddFaceId: {
            if(pAck->ret_code == 0) {
                ...
                if (self.dmsDelegate) {
                    [self.dmsDelegate onAddFaceResult:DMSErrOK];
                }
            } else{
        ...
        case DMS_CMD_RemoveFaceId: {
            ...
            if (pAck->cmd_tag == 0) {
                if (self.dmsDelegate) {
                    [self.dmsDelegate onRemoveFaceResult: (pAck->ret_code!=0) ? DMSErrInternal : DMSErrOK];
                }
            } else  {
                if (self.dmsDelegate) {
                    [self.dmsDelegate onRemoveAllFaceResult:(pAck->ret_code!=0) ? DMSErrInternal : DMSErrOK];
                }
            }
        }
            break;
        case ES_CMD_EstimateCameraPose: {
            ...
            if (self.dmsDelegate) {
                [self.dmsDelegate onCalibResult: (pAck->ret_code!=0) ? DMSErrInternal : DMSErrOK];
            }
        }
            break;
        default:
...
- (void)findNewACKHeader {
    ...
    NSRange totalrange = {0, [self.pRecvBuffer length]};
    NSRange newHeader = [self.pRecvBuffer rangeOfData:dmagic options:NSDataSearchAnchored range:totalrange];
    if (newHeader.length != 0) {
        NSLog(@"find next cmd, location: %ld", (long)newHeader.location);
        NSRange newbufrange = {0, newHeader.location};
        [self.pRecvBuffer replaceBytesInRange:newbufrange withBytes:NULL length:0];
    } else {
        NSLog(@"find no cmd, total length: %ld", (long)[self.pRecvBuffer length]);
        NSRange bufrange = {0, [self.pRecvBuffer length]};
        [self.pRecvBuffer replaceBytesInRange:bufrange withBytes:NULL length:0];
...
- (void)onReciveBuffer:(NSData*)data {
    //deal with all cmd in main queue, do not use lock
    [[NSOperationQueue mainQueue] addOperationWithBlock:^() {
        [self.pRecvBuffer appendData:data];
        while([self.pRecvBuffer length] >= (int)sizeof(dms_ipc_ack_header_t)) {
            dms_ipc_ack_header_t *pAck_s = (dms_ipc_ack_header_t *)[self.pRecvBuffer bytes];
            if(pAck_s->magic == DMS_ACK_MAGIC) {
                NSUInteger actLen = pAck_s->extra_bytes + DMS_ACK_SIZE;
                if([self.pRecvBuffer length] >= actLen) {
                    NSRange ackrange = {0, actLen};
                    [self onProcessACK:[self.pRecvBuffer subdataWithRange:ackrange]];
                    NSRange cmdrange = {0, actLen};
                    [self.pRecvBuffer replaceBytesInRange:cmdrange withBytes:NULL length:0];
                } else {
                    break;
                }
            } else {
