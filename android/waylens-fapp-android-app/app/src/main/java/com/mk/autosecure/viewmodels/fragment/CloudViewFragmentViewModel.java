...
import com.mk.autosecure.ui.fragment.CloudViewFragment;
...
public interface CloudViewFragmentViewModel {
    ...
    final class ViewModel extends FragmentViewModel<CloudViewFragment> implements Inputs, Outputs, Errors, WOWZStatusCallback {

        public static final String TAG = CloudViewFragmentViewModel.ViewModel.class.getSimpleName();
...
//                    .compose(Transformers.pipeErrorsTo(networkError))
        ...
        public void startAudio() {
            ...
            Observable<Response<AudioStreamResponse>> observable;

            if (Constants.isFleet()) {
                observable = ApiClient.createApiService().startAudio(mSerialNumber, streamBody);
            } else {
                observable = ApiService.createApiService().startAudio(mSerialNumber, streamBody);
            }
        ...
        public void endAudio(boolean appPausing) {
            ...
            Observable<AudioStreamResponse> observable;

            if (Constants.isFleet()) {
                observable = ApiClient.createApiService().endAudio(mSerialNumber, streamBody);
            } else {
                observable = ApiService.createApiService().endAudio(mSerialNumber, streamBody);
            }

            observable
                    .subscribeOn(Schedulers.io())
                    .compose(bindToLifecycle())
                    .subscribe(this::onToggleBroadcast, throwable -> {
                        Logger.t(TAG).e("endAudio throwable: " + throwable.getMessage());
...
//                    mWZBroadcastConfig.setPassword("waylensfleet2019");
        ...
        protected synchronized WOWZStreamingError startBroadcast() {
            ...
            if (mWZBroadcast.getStatus().isIdle()) {
                ...
                mWZBroadcastConfig.setAudioSampleRate(44100);
