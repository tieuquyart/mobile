...
public class ParkingSettingFragment extends RxFragment {

    private static final String TAG = ParkingSettingFragment.class.getSimpleName();
    ...
    @BindArray(R.array.parking_event_detection_title)
    String[] detectionTitles;

    @BindArray(R.array.parking_event_detection_content)
    ...
    public static final String[] PARKING_SETTING = new String[]{"off", "low", "medium", "high"};

    public static ParkingSettingFragment newInstance(boolean wifiOnly, String sn) {
        ParkingSettingFragment fragment = new ParkingSettingFragment();
    ...
    public static ParkingSettingFragment newInstance(boolean wifiOnly, CameraBean cameraBean) {
        ParkingSettingFragment fragment = new ParkingSettingFragment();
        Bundle args = new Bundle();
        args.putBoolean(IntentKey.WIFI_ONLY, wifiOnly);
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_park_setting, container, false);
        ...
        if (!TextUtils.isEmpty(sn)) {
            ...
            MountSetting.ModeSetting parkingMode = cameraBean.settings.parkingMode;
            if (parkingMode != null) {
                onMountSettingChangeEvent(parkingMode);
            }
        }

        mstb_alert.setOnValueChangedListener(value -> {
            mstb_alert.setValue(value);//这里如果不设置的话，下面set时候拿到的value是上一次的value，不准确
//            Logger.t(TAG).e("mstb_alert onValueChanged");
        ...
        mstb_upload.setOnValueChangedListener(value -> {
            ...
            Logger.t(TAG).e("mstb_upload onValueChanged");
            setMountSetting("uploadSensitivity", value);
    ...
    private void setUploadText(int value) {
        if (value >= 0 && value < detectionTitles.length) {
            ...
            if (value == 3) {
                tv_uploadContent.setText(Html.fromHtml(uploadContents[value] + getString(R.string.increase_data_usage)));
            } else {
    ...
    public void onMountSettingChangeEvent(MountSettingChangeEvent event) {
        if (event != null) {
            ...
            MountSetting.ModeSetting parkingMode = event.getMountSetting().parkingMode;
            if (parkingMode != null) {
                Logger.t(TAG).e("cameraSetting: " + parkingMode.monitoring
                        + "--detection: " + parkingMode.detectionSensitivity
                        + "--alert: " + parkingMode.alertSensitivity
                        + "--upload: " + parkingMode.uploadSensitivity);

                if (wifiOnly) {
                    int detectIndex;
                    if ("off".equals(parkingMode.monitoring)) {
                        detectIndex = 0;
                    } else {
                        detectIndex = getIndex(parkingMode.detectionSensitivity);
                    }
                    ...
                    if ("off".equals(parkingMode.monitoring)) {
                        ...
                        detectIndex = getIndex(parkingMode.detectionSensitivity);
                        uploadIndex = getIndex(parkingMode.uploadSensitivity);
                    }
                    mstb_alert.setValue(detectIndex);
    ...
    private void onMountSettingChangeEvent(MountSetting.ModeSetting parkingMode) {
        Logger.t(TAG).e("cameraSetting: " + parkingMode.monitoring
                + "--detection: " + parkingMode.detectionSensitivity
                + "--alert: " + parkingMode.alertSensitivity
                + "--upload: " + parkingMode.uploadSensitivity);
    ...
    private int getIndex(String sensitivity) {
        int index;
        switch (sensitivity) {
            case "low":
                index = 1;
    ...
    private void setMountSetting(String item, int index) {
        try {
            if (mCamera != null) {
                String sensitivity = "";
                if (index >= 0 && index < PARKING_SETTING.length) {
                    sensitivity = PARKING_SETTING[index];
                }
                if (TextUtils.isEmpty(sensitivity)) {
                    return;
                }
                JSONObject inModeSetting = new JSONObject();
                JSONObject parkSetting = new JSONObject();
                inModeSetting.put(item, sensitivity);
                parkSetting.put("parkingMode", inModeSetting);

                Logger.t(TAG).e("parkSetting = %s", parkSetting.toString());

                mCamera.setMountSettings(parkSetting.toString());
            }
    ...
    public String getDetection() {
        if (mstb_alert != null) {
            return PARKING_SETTING[mstb_alert.getValue()];
        }
        return PARKING_SETTING[0];
    }

    public String getUpload() {
        if (mstb_upload != null) {
            return PARKING_SETTING[mstb_upload.getValue()];
        }
        return PARKING_SETTING[0];
    }
}
