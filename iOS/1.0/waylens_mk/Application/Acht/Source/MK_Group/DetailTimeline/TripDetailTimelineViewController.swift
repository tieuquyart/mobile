...
//    var tripDetailCells : [TripDetailTimeLineTableViewCell]?
...
//    var tripList = [Trip]()
    ...
    override func applyTheme() {
        ...
        setTextLabelLocalized(label: self.detailLabel, text: "Make Call")
    ...
    func updateData(_ trips: [Trip]){
        ...
        mileageLabel.text = mileages.measurementLength(unit: .kilometers).localeStringValue
        
        eventLabel.text = "\(eventCounts)"
        
        hourLable.text = hours.measurementDuration(unit: .hours).localeStringValue
    }
...
extension TripDetailTimelineViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cellTrip.setBorderWithExpanded(isExpanded: self.allItems[indexPath.row].isExpanded)
        if indexPath.row == 0 {
            let model = allItems[indexPath.section].trip
            print("modelTrip: \(model)")
