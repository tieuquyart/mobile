...
//                        case .success(let value):
//
//                            if let status = HNLiveStatus(rawValue: value["status"] as! String) {
//
//                                if let url = value["playUrl"] as? String {
//                                    self?.liveUrl = url
//                                    completion?(result)
...
//            WaylensClientS.shared.fetchLiveStatus(self.sn, completion: { (result) in
//                if result.isSuccess, let status = HNLiveStatus(rawValue: result.value!["status"] as! String) {
//                    if let url = result.value?["playUrl"] as? String {
//                        self.liveUrl = url
//                    }
//                    if self.liveStatus != status {
//                        self.liveStatus = status
            ...
            FleetViewService.shared.upload_status(cameraSn: sn) { [unowned self] (result) in
                if result.isSuccess {
                    if let dict = result.value {
                        #if FLEET
                        self.uploadingSpeedBitps = dict["bytesInRate"] as? Int
                        #else
...
//            WaylensClientS.shared.fetchLiveBitRate(sn) { [unowned self] (result) in
//                if result.isSuccess {
//                    if let dict = result.value {
//                        #if FLEET
//                        self.uploadingSpeedBitps = dict["bytesInRate"] as? Int
