...
void dd_didSelectRowAtIndexPath(id self, SEL _cmd, id tableView, NSIndexPath *indexpath) {
    ...
    if (infoDictionary == nil) {
        ...
        dict = [infoDictionary mutableCopy];
    }
    dict[@"index_path"] = [NSString stringWithFormat:@"%ld-%ld", (long)indexpath.section, indexpath.row];
    if (cell.textLabel.text.length) {
        dict[@"title_text"] = cell.textLabel.text;
    }
