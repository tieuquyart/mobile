...
//  Acht
//
//  Created by forkon on 2020/3/24.
...
extension UIButton {
            ...
            if newValue {
                ...
                rx.methodInvoked(#selector(UIView.traitCollectionDidChange(_:))).subscribe { [weak self] (event) in
                    ...
                    if let semanticLabelColor = self.semanticLabelColor {
                        self.setTitleColor(UIColor.semanticColor(.label(semanticLabelColor)), for: .normal)
                    }
    ...
    private func replaceTextColorWithDynamicColor() {
        LabelColor.allCases.forEach { (labelColor) in
            if titleColor(for: .normal)?.isSameColorAs(UIColor.semanticColor(.label(labelColor), usingTheme: LightTheme())) == true {
                semanticLabelColor = labelColor
                self.setTitleColor(UIColor.semanticColor(.label(labelColor)), for: .normal)
                return
            }
        }
    }
