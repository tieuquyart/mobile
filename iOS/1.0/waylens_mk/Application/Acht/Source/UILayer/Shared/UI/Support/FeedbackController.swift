...
class FeedbackController: BaseViewController {
    ...
    @IBOutlet weak var sendButton: HNMainButton!
...
//        var logData: Data?
        if logs.count > 0 {
            do {
                logUrl = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("logs.zip")
