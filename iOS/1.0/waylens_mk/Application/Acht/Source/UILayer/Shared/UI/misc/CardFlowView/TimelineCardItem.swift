...
public class TimelineCardItem {
    ...
    public struct TextElement {
        ...
        init(text: String, font: UIFont) {
            self.text = text
            self.font = font
            self.color = { UIColor.semanticColor(.label(.secondary)) }
        }
    }
...
extension TimelineCardItem.TextElement {
    ...
    static func titleElement(_ title: String) -> TimelineCardItem.TextElement {
        return TimelineCardItem.TextElement(text: title, font: UIFont.systemFont(ofSize: 14), color: { UIColor.semanticColor(.label(.secondary)) })
    }

    static func abstractElement(_ abstract: String) -> TimelineCardItem.TextElement {
        return TimelineCardItem.TextElement(text: abstract, font: UIFont.systemFont(ofSize: 14), color: { UIColor.semanticColor(.label(.primary)) })
    }
