...
class ObserverForCameraDetail: Observer {

    weak var eventResponder: ObserverForCameraDetailEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<CameraDetailViewControllerState>
    ...
    init(state: Observable<CameraDetailViewControllerState>) {
        self.state = state
    }
