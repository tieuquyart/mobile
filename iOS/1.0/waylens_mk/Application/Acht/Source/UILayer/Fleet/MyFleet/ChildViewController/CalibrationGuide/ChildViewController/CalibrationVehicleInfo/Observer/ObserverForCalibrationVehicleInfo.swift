//
//  ObserverForCalibrationVehicleInfo.swift
//  Fleet
//
...
class ObserverForCalibrationVehicleInfo: Observer {

    weak var eventResponder: ObserverForCalibrationVehicleInfoEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<CalibrationVehicleInfoViewControllerState>
    ...
    init(state: Observable<CalibrationVehicleInfoViewControllerState>) {
        self.state = state
    }
