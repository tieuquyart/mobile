...
class ObserverForPowerInfo: Observer {

    weak var eventResponder: ObserverForPowerInfoEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<PowerInfoViewControllerState>
    ...
    init(state: Observable<PowerInfoViewControllerState>) {
        self.state = state
    }
