...
class ObserverForSetupSuccess: Observer {

    weak var eventResponder: ObserverForSetupSuccessEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<SetupSuccessViewControllerState>
    ...
    init(state: Observable<SetupSuccessViewControllerState>) {
        self.state = state
    }
