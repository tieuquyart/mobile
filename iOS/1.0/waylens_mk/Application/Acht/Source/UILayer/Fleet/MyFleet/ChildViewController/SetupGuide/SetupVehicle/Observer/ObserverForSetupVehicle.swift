...
class ObserverForSetupVehicle: Observer {

    weak var eventResponder: ObserverForSetupVehicleEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<SetupVehicleViewControllerState>
    ...
    init(state: Observable<SetupVehicleViewControllerState>) {
        self.state = state
    }
