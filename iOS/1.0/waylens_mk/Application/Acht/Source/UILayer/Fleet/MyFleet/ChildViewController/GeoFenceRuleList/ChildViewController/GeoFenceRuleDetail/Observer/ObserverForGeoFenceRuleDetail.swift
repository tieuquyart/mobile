//
//  ObserverForGeoFenceRuleDetail.swift
//  Fleet
//
...
class ObserverForGeoFenceRuleDetail: Observer {

    weak var eventResponder: ObserverForGeoFenceRuleDetailEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<GeoFenceRuleDetailViewControllerState>
    ...
    init(state: Observable<GeoFenceRuleDetailViewControllerState>) {
        self.state = state
    }
