...
class ObserverForCamera: Observer {

    weak var eventResponder: ObserverForCameraEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<CameraViewControllerState>
    ...
    init(state: Observable<CameraViewControllerState>) {
        self.state = state
    }
