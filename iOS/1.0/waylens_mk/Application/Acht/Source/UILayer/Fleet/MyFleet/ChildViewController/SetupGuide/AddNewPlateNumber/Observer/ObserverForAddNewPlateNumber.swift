//
//  ObserverForAddNewPlateNumber.swift
//  Fleet
//
...
class ObserverForAddNewPlateNumber: Observer {

    weak var eventResponder: ObserverForAddNewPlateNumberEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<AddNewPlateNumberViewControllerState>
    ...
    init(state: Observable<AddNewPlateNumberViewControllerState>) {
        self.state = state
    }
