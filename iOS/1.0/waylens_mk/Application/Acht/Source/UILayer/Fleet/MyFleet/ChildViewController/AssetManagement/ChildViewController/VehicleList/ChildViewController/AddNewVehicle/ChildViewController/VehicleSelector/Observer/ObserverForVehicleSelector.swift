//
//  ObserverForVehicleSelector.swift
//  Fleet
//
...
class ObserverForVehicleSelector: Observer {

    weak var eventResponder: ObserverForVehicleSelectorEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<VehicleSelectorViewControllerState>
    ...
    init(state: Observable<VehicleSelectorViewControllerState>) {
        self.state = state
    }
