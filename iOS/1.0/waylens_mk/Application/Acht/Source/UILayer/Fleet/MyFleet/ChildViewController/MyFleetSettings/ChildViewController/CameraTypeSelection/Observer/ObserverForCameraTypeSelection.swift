//
//  ObserverForCameraTypeSelection.swift
//  Fleet
//
...
class ObserverForCameraTypeSelection: Observer {

    weak var eventResponder: ObserverForCameraTypeSelectionEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<CameraTypeSelectionViewControllerState>
    ...
    init(state: Observable<CameraTypeSelectionViewControllerState>) {
        self.state = state
    }
