...
class CheckCameramMKViewController: BaseViewController {
    ...
    @IBOutlet weak var serialCameraTextField: UITextField!
    ...
    func config() {
        guard let serialCamera = serialCameraTextField.text, !serialCamera.isEmpty else {
                    ...
                    if let success = dict["success"] as? Bool {
                        ...
                        if success {
                            ...
                            if let mess = dict["message"] as? String {
                                strongSelf.showAlert(title: ConstantMK.language(str: "Alert"), message: ConstantMK.language(str: mess))
                            }
                        }
                    } else {
                        HNMessage.showError(message: _result.error?.localizedDescription ?? NSLocalizedString("Login Failed", comment: "Login Failed"), to: strongSelf.navigationController)
                    }
                    break
    ...
    override func applyTheme() {
        ...
        self.viewAccount.layer.cornerRadius = 8.0
        self.viewAccount.addShadow(offset: CGSize.init(width: 3, height: 4))
   }
