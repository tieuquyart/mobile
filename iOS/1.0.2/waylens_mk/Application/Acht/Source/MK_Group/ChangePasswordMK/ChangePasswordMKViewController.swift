...
class ChangePasswordMKViewController: BaseViewController {
    ...
    override func applyTheme() {
        ...
        oldPassword.txtSecureEntry(secureTxt: true)
        newPassword.txtSecureEntry(secureTxt: true)
        repeatPassword.txtSecureEntry(secureTxt: true)
    ...
    func isValidate() -> Bool {
        if self.oldPassword.infoTextField.text == "" {
            self.toastMessage(message: "Mời nhập mật khẩu cũ")
            return false
        } else if self.newPassword.infoTextField.text == "" {
            self.toastMessage(message: "Mời nhập mật khẩu mới")
            return false
        } else if self.repeatPassword.infoTextField.text == "" {
            self.toastMessage(message: "Mời nhập lại mật khẩu mới")
            return false
        } else if (self.newPassword.infoTextField.text?.count ?? 0) < 8 {
            self.toastMessage(message: "Mật khẩu từ 8 đến 30 ký tự")
            return false
        } else if (self.newPassword.infoTextField.text?.count  ?? 0) > 30 {
            self.toastMessage(message: "Mật khẩu từ 8 đến 30 ký tự")
            return false
        } else if self.newPassword.infoTextField.text !=  self.repeatPassword.infoTextField.text {
    ...
    func onSave() {
        ...
        //            return
        //        }
        SessionService.shared.changePassword(newPassword: newPassword.infoTextField.text ?? "" , oldPassword: oldPassword.infoTextField.text ?? "", completion: { [weak self] (result) in
            if result.isSuccess {
