...
public enum DurationStyleType {
    case hms
    case colon
}

public extension TimeInterval {
    func toString(_ style:DurationStyleType) -> String {
        var remaining = max(0, Int(self + 0.5))
        ...
        case .hms:
            if hours > 0 {
                return String(format: NSLocalizedString("%dh%dm%ds", comment: "%dh%dm%ds"), hours, minutes, seconds)
            } else if minutes > 0 {
                return String(format: NSLocalizedString("%dm%ds", comment: "%dm%ds"), minutes, seconds)
            } else {
                return String(format: NSLocalizedString("%ds", comment: "%ds"), seconds)
            }
        case .colon:
            if hours > 0 {
    ...
    func toHourMinSecString() -> String {
        var remaining = max(0, Int(self + 0.5))
        let hours = remaining / 3600
        remaining -= hours * 3600
        let minutes = remaining / 60
        ...
        if hours != 0 {
            readableString += String(format: NSLocalizedString("%dhour", comment: "%dhour"), hours)
        }

        if minutes != 0 {
            readableString += String(format: NSLocalizedString("%dmin", comment: "%dmin"), minutes)
        }

        if seconds != 0 {
            readableString += String(format: NSLocalizedString("%dsec", comment: "%dsec"), seconds)
        }
...
public extension Date {
    ...
    var dateAtStartOfDay: Date {
        let secondsSince1970 = timeIntervalSince1970
        let dayInSeconds: TimeInterval = 24 * 60 * 60
        return Date(timeIntervalSince1970: secondsSince1970 - secondsSince1970.truncatingRemainder(dividingBy: dayInSeconds))
    }
...
public extension Date {
    ...
    func inDefaultRegion() -> DateInRegion {
        return DateInRegion(self, region: Region.local)
    }

    func toHumanizedDateString() -> String {
        if inDefaultRegion().compare(.isToday) {
            return NSLocalizedString("Today", comment: "Today")
        } else if inDefaultRegion().compare(.isYesterday) {
            return NSLocalizedString("Yesterday", comment: "Yesterday")
        } else if inDefaultRegion().compare(.isThisYear) {
            let format = Date.cachedFormatter(DateFormatType.dateMonthDay.stringFormat)
            return format.string(from: self)
        } else {
            let format = Date.cachedFormatter(DateFormatType.dateYearMonthDay.stringFormat)
            return format.string(from: self)
        }
    }
    ...
    func toHumanizedDateString() -> String {
        if self.compare(.isToday) {
            ...
            return NSLocalizedString("Yesterday", comment: "Yesterday")
        } else if self.compare(.isThisYear) {
            let format = Date.cachedFormatter(DateFormatType.dateMonthDay.stringFormat)
            return format.string(from: self)
        } else {
            let format = Date.cachedFormatter(DateFormatType.dateYearMonthDay.stringFormat)
    ...
    func toHumanizedDateTimeString() -> String {
        let time = self.toString(format: .timeMin12)
        if self.compare(.isToday) {
            return time
        } else {
            return "\(time) \(toHumanizedDateString())"
        }
    }

    func toStringUsingInMessageCell() -> String {
        ...
        if sec < 60 {
            if sec < 10 {
                ...
                let string:String
                if isPast {
                    string = NSLocalizedString("%.f seconds ago", comment: "Date format")
                } else {
                    string = NSLocalizedString("in %.f seconds", comment: "Date format")
                }
        ...
        if min < 60 {
            if min == 1 {
                ...
                let string:String
                if isPast {
                    string = NSLocalizedString("%.f minutes ago", comment: "Date format")
                } else {
                    string = NSLocalizedString("in %.f minutes", comment: "Date format")
                }
        ...
        if hr < 24 {
            if hr == 1 {
                ...
                let string:String
                if isPast {
                    string = NSLocalizedString("%.f hours ago", comment: "Date format")
                } else {
                    string = NSLocalizedString("in %.f hours", comment: "Date format")
                }
    ...
    func compare(_ comparison:DateComparisonType) -> Bool {
        ...
        case .isTomorrow:
            ...
            return compare(.isSameDay(comparison))
        case .isYesterday:
            let comparison = Date().adjust(.day, offset: -1)
            return compare(.isSameDay(comparison))
...
public enum DateFormatType {
    ...
    var stringFormat:String {
        ...
        case .timeSec12: return NSLocalizedString("h:mm:ss a", comment: "h:mm:ss a")
        case .timeMin12: return NSLocalizedString("h:mm a", comment: "h:mm a")
        case .dateTimeSec: return "yyyy-MM-dd_HH:mm:ss"
        case .dashCam: return "yyyy.MM.dd HH:mm:ss"
        case .dateMonthDay: return NSLocalizedString("MMM d", comment: "MMM d")
        case .dateYearMonthDay: return NSLocalizedString("MMM d, yyyy", comment: "MMM d, yyyy")
...
public enum TimeZoneType {
    case local, utc, custom(_ tz: TimeZone)
