...
class ObserverForRecordConfig: Observer {

    weak var eventResponder: ObserverForRecordConfigEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<RecordConfigViewControllerState>
    ...
    init(state: Observable<RecordConfigViewControllerState>) {
        self.state = state
    }
