...
class ObserverForVehicle: Observer {

    weak var eventResponder: ObserverForVehicleEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<VehicleViewControllerState>
    ...
    init(state: Observable<VehicleViewControllerState>) {
        self.state = state
    }
