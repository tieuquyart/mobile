...
class ObserverForBindDriver: Observer {

    weak var eventResponder: ObserverForBindDriverEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<BindDriverViewControllerState>
    ...
    init(state: Observable<BindDriverViewControllerState>) {
        self.state = state
    }
