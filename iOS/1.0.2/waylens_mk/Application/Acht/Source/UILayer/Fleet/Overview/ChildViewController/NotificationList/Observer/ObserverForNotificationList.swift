//
//  ObserverForNotificationList.swift
//  Fleet
//
...
class ObserverForNotificationList: Observer {

    weak var eventResponder: ObserverForNotificationListEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<NotificationListViewControllerState>
    ...
    init(state: Observable<NotificationListViewControllerState>) {
        self.state = state
    }
