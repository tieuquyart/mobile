//
//  ObserverForAddNewGeoFence.swift
//  Fleet
//
...
class ObserverForAddNewGeoFence: Observer {

    weak var eventResponder: ObserverForAddNewGeoFenceEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<AddNewGeoFenceViewControllerState>
    ...
    init(state: Observable<AddNewGeoFenceViewControllerState>) {
        self.state = state
    }
