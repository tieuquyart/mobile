...
class MyFleetSettingsDependencyContainer {
    ...
    func makeMyFleetSettingsViewController() -> MyFleetSettingsViewController {
        ...
        let userInterface = MyFleetSettingsRootView()
        let vc = MyFleetSettingsViewController(observer: observer, userInterface: userInterface, viewControllerFactory: self)
        observer.eventResponder = vc
