...
class CalibrationInstallationPositionContentView: UIView, Themed {
    ...
    func applyTheme() {
        installationGuideButton.setTitleColor(UIColor.semanticColor(.tint(.primary)), for: .normal)
        installationGuideButton.addUnderline(with: UIColor.semanticColor(.tint(.primary)))
        warningLabel.textColor = UIColor.semanticColor(.label(.primary))
    }
