//
//  ObserverForGeoFenceDrawing.swift
//  Fleet
//
...
class ObserverForGeoFenceDrawing: Observer {

    weak var eventResponder: ObserverForGeoFenceDrawingEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<GeoFenceDrawingViewControllerState>
    ...
    init(state: Observable<GeoFenceDrawingViewControllerState>) {
        self.state = state
    }
