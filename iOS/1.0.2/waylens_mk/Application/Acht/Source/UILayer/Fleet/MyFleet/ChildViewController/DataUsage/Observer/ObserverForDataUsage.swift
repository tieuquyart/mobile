...
class ObserverForDataUsage: Observer {

    weak var eventResponder: ObserverForDataUsageEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<DataUsageViewControllerState>
    ...
    init(state: Observable<DataUsageViewControllerState>) {
        self.state = state
    }
