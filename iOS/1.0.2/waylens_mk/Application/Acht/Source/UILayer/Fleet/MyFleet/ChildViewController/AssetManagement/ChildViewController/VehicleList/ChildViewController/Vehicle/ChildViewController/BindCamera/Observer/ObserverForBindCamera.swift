...
class ObserverForBindCamera: Observer {

    weak var eventResponder: ObserverForBindCameraEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<BindCameraViewControllerState>
    ...
    init(state: Observable<BindCameraViewControllerState>) {
        self.state = state
    }
