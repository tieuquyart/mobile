...
class FetchBillingDataUseCase: UseCase {
    ...
    public func start() {
        ...
        WaylensClientS.shared.request(
            ...
            case .success(let response):
                if let billingData = try? JSONDecoder().decode(BillingData.self, from: response.jsonData ?? Data()) {
                    self.actionDispatcher.dispatch(DataUsageActions.loadBillingData(billingData))
                }
...
protocol FetchBillingDataUseCaseFactory {
