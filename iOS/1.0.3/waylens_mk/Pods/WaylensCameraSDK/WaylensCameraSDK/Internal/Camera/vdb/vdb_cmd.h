...
//-----------------------------------------------------------------------
//
//	1. each cmd is 160 bytes (padded with 0);
//	2. for each ack, there's a 32-byte header (vdb_ack_t);
//	3. if ack's size is less than 160 bytes, it's padded to 160 with 0;
//	4. if ack's size is more than 160 bytes, server sents 160 bytes first, and then
//		a variable size block. The block's size is specified in vdb_ack_t.extra_bytes
//	5. msg is same with ack
//
//-----------------------------------------------------------------------


#define VDB_VERSION_MAJOR	1
#define VDB_VERSION_MINOR	11

#define VDB_CMD_PORT		8083
#define VDB_HTTP_PORT		8085

#define CLIPINFO_FILE_EXT	".clip"
#define CLIPINFO_FILE_EXT2	".temp"
#define INDEX_FILE_EXT		".idx"
#define VIDEO_FILE_TS_EXT	".ts"
#define VIDEO_FILE_MP4_EXT	".mp4"
#define VIDEO_FILE_MJPEG_EXT ".jpg"
...
enum {
	...
	// since version 1.9
	//-----------------------
	VDB_CMD_GetExtraRawData = 60,
	VDB_CMD_GetExtraRawDataBlock = 61,

	//-----------------------
	// since version 1.10
	//-----------------------
	VDB_CMD_GetVideoFileList = 70,
	...
	// since version 1.10
	//-----------------------
	VDB_MSG_RawDataEx = _VDB_MSG_START + 100,
	VDB_MSG_AdjustRawDataTS = _VDB_MSG_START + 101,
};
...
#define DOWNLOAD_OPT_SUB_STREAM_N	(1 << 7)	// stream number in upper 16 bits of download_opt
#define DOWNLOAD_OPT_SUB_N_MP4		(1 << 8)
...
typedef struct vdb_cmd_GetDownloadUrlEx_s {
	...
	uint32_t download_opt;		// extra option; upper 16 bits are vn_stream
	//	if (VDB_HAS_ID) {
	//		vdb_id_t vdb_id;
	//	}
...
//-----------------------------------------------------------------------
//
//	VDB_CMD_GetExtraRawData
//
//-----------------------------------------------------------------------

typedef struct vdb_cmd_GetExtraRawData_s {
	vdb_cmd_header_t header;	// must be first
	uint32_t clip_type;
	uint32_t clip_id;
	uint32_t clip_time_ms_lo;	// clip_start_time_ms + time_offset
	uint32_t clip_time_ms_hi;
	uint32_t all_data_types;	// 1: get all, ignore data_types; 0: use data_types
	uint32_t data_types;		// bit 0: 0-th raw data in clip;
	// bit 1: 1st raw data in clip; etc
	// see GET_CLIP_RAW_FCC
	...
	//		vdb_id_t vdb_id;
	//	}
} vdb_cmd_GetExtraRawData_t;

typedef struct vdb_ack_GetExtraRawData_s {
	uint32_t clip_type;
	uint32_t clip_id;
	uint32_t clip_date;		// seconds from 1970 (when the clip was created)

	uint32_t data_types;	// output types
...
#define RAW_DATA_F_NEED_TS		(1 << 0)
#define RAW_DATA_F_UDP			(1 << 1)	// send raw data to UDP port

typedef struct vdb_cmd_SetRawDataOption_s {
	vdb_cmd_header_t header;	// must be first
	uint32_t data_types;		// (1<<kRawData_GPS) | (1<<kRawData_ACC) | (1<<kRawData_OBD)
	uint32_t flags;				// RAW_DATA_F_NEED_TS | RAW_DATA_F_UDP
	uint32_t param;				// UDP port: if RAW_DATA_F_UDP is set
...
#define DATA_OPTION_NO_1ST_ITEM		(1 << 3)	// do not read the 1st item in range
...
//-----------------------------------------------------------------------
//
//	VDB_CMD_GetExtraRawDataBlock
//
//-----------------------------------------------------------------------

typedef struct vdb_cmd_GetExtraRawDataBlock_s {
	vdb_cmd_header_t header;
	...
	uint32_t raw_fcc;			//
	uint32_t data_options;		// DATA_OPTION_NO_DATA|DATA_OPTION_NO_INDEX|DATA_OPTION_DOWNSAMPLING
...
} vdb_cmd_GetExtraRawDataBlock_t;

typedef struct vdb_ack_GetExtraRawDataBlock_s {
	uint32_t clip_type;
	uint32_t clip_id;
	uint32_t clip_date;		// seconds from 1970 (when the clip was created)

	uint32_t raw_fcc;		// same as cmd
	uint32_t data_options;	// same as cmd
...
} vdb_ack_GetExtraRawDataBlock_t;
...
#define UPLOAD_GET_EXRAW_DATA	(1 << 6)	// extra raw data
#define UPLOAD_REMOVE_A0		(1 << 8)
#define UPLOAD_REMOVE_A1		(1 << 9)
#define UPLOAD_REMOVE_AN		(1 << 10)
#define UPLOAD_GET_VN			(1 << 11)		// specify VN instread of V1, set vn_stream
...
#define UPLOAD_GET_DATA			(UPLOAD_GET_PIC|UPLOAD_GET_RAW|UPLOAD_GET_EXRAW_DATA)
#define UPLOAD_GET_VIDEO		(UPLOAD_GET_V0|UPLOAD_GET_V1|UPLOAD_GET_VN)
...
typedef struct vdb_cmd_GetUploadUrl_s {
	...
	uint32_t exraw_masks;		// mask for extra raw data; bits correspond to clip FCCs
	uint32_t vn_stream;			// stream 2..n-1
	//	if (VDB_HAS_ID) {
	//		vdb_id_t vdb_id;
	//	}
...
typedef struct vdb_ack_GetUploadUrl_s {
	...
	uint32_t exraw_masks;	// return the cmd param
	uint32_t vn_stream;		// return the cmd param
...
enum {
	VDB_OPTION_NONE = 0,
	VDB_OPTION_HLS_SEGMENT_LENGTH = 1,	// param[0] is hls segment length
	VDB_OPTION_TCP_NODELAY = 2,			// param[0] is 0 or 1
	VDB_OPTION_ENABLE_UDP_FOR_RAWDATA = 3,	// param[0]: 1 if param[1] is valid
	// param[1]: data_types: (1<<kRawData_GPS) | (1<<kRawData_ACC) | (1<<kRawData_OBD)
	VDB_OPTION_DISABLE_UDP_FOR_RAWDATA = 4,
	VDB_OPTION_NEED_EXTRA_RAW_DATA = 5,	// request to send extra raw data, param[0] is fcc
};
...
// vdb_clip_info_ex_t.inherited.flags (16-bit)
...
#define GET_CLIP_RAW_FCC	(1 << 7)	// get extra raw data FCCs
#define GET_CLIP_DATE_OFF	(1 << 8)	// get clip date offset (in us)
#define GET_CLIP_VIDEO_TYPE	(1 << 9)
#define GET_CLIP_VIDEO_DESCR	(1 << 10)	// get video stream description
...
#define CLIP_ATTR_NO_MARK			(1 << 7)	// cannot mark in this clip

#define CLIP_ATTR_APP_A				(1 << 12)
#define CLIP_ATTR_APP_B				(1 << 13)
#define CLIP_ATTR_APP_C				(1 << 14)
#define CLIP_ATTR_APP_D				(1 << 15)
...
typedef struct vdb_clip_info_ex_s {
	...
	//	if (inherited.flags & GET_CLIP_DATE_OFF) {
	//		uint32_t	clip_date_high;		// high 32-bit of clip_date
	//		uint32_t	clip_date_offset_us;
	//	}

	//	if (inherited.flags & GET_CLIP_VIDEO_TYPE) {
	//		for (int i = 0; i < inherited.num_streams; i++) {
	//			uint8_t		video_type;		// avf_std_media.h, VIDEO_TYPE_TS or VIDEO_TYPE_MP4
	//			uint8_t		reserved[3];	// 4 bytes per stream
	//		}
	//	}

	//	if (inherited.flags & GET_CLIP_VIDEO_DESCR) {
	//		for (int i = 0; i < inherited.num_streams; i++) {
	//			uint32_t descr_len;			// not including trailing 0
	//			if (descr_len != 0) {
	//				uint8_t descr[desc_len];	// description
	//				uint8_t padding_0[align_to_4n_bytes];
	//			}
	//		}
	//	}
...
typedef struct vdb_cmd_set_clip_data_s {
	vdb_cmd_header_t header;        // must be first
	uint32_t clip_type;
	uint32_t clip_id;
	uint16_t type;          // should be 0
	uint16_t options;       // should be 0
	uint32_t reserved;      // should be 0
	uint32_t data_size;
	//	uint8_t data[data_size];
} vdb_cmd_set_clip_data_t;
...
//-----------------------------------------------------------------------
//
//  VDB_CMD_GetVideoFileList
//
//-----------------------------------------------------------------------
typedef struct vdb_cmd_GetVideoFileList_s {
	vdb_cmd_header_t header;	// must be first
	uint32_t clip_type;
	uint32_t clip_id;
	uint32_t stream;			// stream 0..n-1
	uint32_t url_type;			// not used, set to 0
	uint32_t start_time_ms_lo;
	uint32_t start_time_ms_hi;
	uint32_t length_ms;
	uint32_t b_playlist;
	uint32_t options;			// set to 0
	//	if (VDB_HAS_ID) {
	//		vdb_id_t vdb_id;
	//	}
} vdb_cmd_GetVideoFileList_t;

typedef struct vdb_ack_GetVideoFileList_s {
	uint32_t clip_type;
	uint32_t clip_id;
	uint32_t stream;			// STREAM_MAIN, STREAM_SUB_1
	uint32_t url_type;			// not used
	uint32_t real_time_ms_lo;
	uint32_t real_time_ms_hi;
	uint32_t length_ms;
	uint32_t b_playlist;
	uint32_t options;
	uint32_t list_size;			// including trailing 0

	//	uint8_t file_list[list_size];

	//	if (VDB_HAS_ID) {
	//		vdb_id_t vdb_id;
	//	}
} vdb_ack_GetVideoFileList_t;
...
typedef struct vdb_msg_RawData_s {
	vdb_ack_t header;		// must be first
	uint32_t data_type;		// kRawData_GPS, kRawData_ACC, kRawData_OBD; or fcc
	uint32_t data_size;
...
//
//-----------------------------------------------------------------------
typedef struct vdb_msg_RawDataEx_s
{
	vdb_ack_t header;		// must be first
	uint32_t data_type;		// kRawData_GPS, kRawData_ACC, kRawData_OBD; or fcc
	uint32_t data_size;
	...
	//	uint8_t extra_data[extra_size];	// reserved for future use

	//	uint8_t data[data_size];	// raw data
	//	if (VDB_HAS_ID) {
	//		vdb_id_t vdb_id;
	//	}
} vdb_msg_RawDataEx_t;
...
//-----------------------------------------------------------------------
//
//  VDB_MSG_AdjustRawDataTS
//
//-----------------------------------------------------------------------
typedef struct VDB_MSG_AdjustRawDataTS_s {
	vdb_ack_t header;	// must be first
	uint32_t delta_ts_ms_lo;	// lo + hi -> int64_t delta_ts
	uint32_t delta_ts_ms_hi;
} VDB_MSG_AdjustRawDataTS_t;
