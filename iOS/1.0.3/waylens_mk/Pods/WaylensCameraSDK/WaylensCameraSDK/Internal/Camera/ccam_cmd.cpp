	...
	): _num(0)
{
    ...
    if((p1 != NULL)&&( p2 != NULL)&&(p2 > p1))
    {
        ...
        doc = xmlReadMemory(p1, (int)(p2 + strlen(envelop_root_element_name) + 2 - p1), "noname.xml", NULL, 0);
        if (doc == NULL) {
...
int	EnumedStringCMD::getDomain()
{
    int rt = -1;
    char *p;
    if((p = strstr(getName(), EnumCMDPreFix)) != NULL)
    {
...
int EnumedStringCMD::getCMD()
{
    int rt = -1;
    char *p;
    if((p = strstr(getName(), EnumCMDBreak)) != NULL)
    {
        //printf("-- get cmd %s, %s\n", getName(), p);
...
void EnumedStringCMD::Send(EnumedStringCMD* tricker)
{
    ...
    if(tricker != NULL)
    {
        tricker->getResponsSendBack()->SendBack(ev.getBuffer(), ev.getBufferLen());
    }
    else
    {
