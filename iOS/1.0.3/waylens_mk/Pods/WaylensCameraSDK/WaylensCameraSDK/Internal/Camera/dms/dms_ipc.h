...
enum {
    ...
    ES_CMD_GetDumpYFlag = ES_CMD_START + 3,
    ES_CMD_SetDumpYFlag = ES_CMD_START + 4,
...
typedef struct es_cmd_GetDumpYflag_s {
    dms_ipc_cmd_header_t    header;    // must be first
} es_cmd_GetDumpYflag_t;

typedef struct es_ack_GetDumpYflag_s {
    uint32_t flag;
} es_ack_GetDumpYflag_t;
...
typedef struct es_cmd_SetDumpYflag_s {
    dms_ipc_cmd_header_t    header;    // must be first
    uint32_t flag;
} es_cmd_SetDumpYflag_t;
