...
class DMSFaceIDList : NSObject, WLSocketClientConnectionDelegate, WLDmsClientDelegate,
    ...
    func addCamera(id : UInt64 , userName : String ) {
        ...
        self.inited = false
        self.superViewController?.alert(title: "Thông báo", message: "Gửi lệnh thêm face thành công")
        //  self.tabView?.reloadData()
    }
        ...
        case 0: do {
            ...
            if let faceId = faceId(in: dict) {
                ...
                let Create = UIAlertAction(title: "Đồng ý", style: .default, handler: { (action) -> Void in
                    ...
                    CameraService.shared.deleteFaceIdToServer(cameraSn: self.cameraUnified?.sn ?? "") { [weak self] (_result) in
                        if let strongSelf = self {
                            strongSelf.showProgress(val: false)
                            switch _result {
                            case .success(let value):
                                ConstantMK.parseJson(dict: value, handler: {success, msg, code in
                                    if success{
                                        strongSelf.dmsClient?.removeFace(withID: faceId)
                                    }else{
                                        HNMessage.showError(message: msg)
                                    }
                                    strongSelf.showProgress(val: false)
                                })
                ...
                let Create = UIAlertAction(title: "Thêm", style: .default, handler: { (action) -> Void in
                    if let cccdInput = self.cccdTextField!.text {
                        if cccdInput == "" {
                            ...
                            CameraService.shared.updateFaceIdToServer(cameraSn: self.cameraUnified?.sn ?? "", numberId: cccdInput, faceId: "\(self.id ?? 0)") { [weak self] (_result) in
                                if let strongSelf = self {
                                        ...
                                        ConstantMK.parseJson(dict: value, handler: {success, msg, code in
                                            if success{
                                                if let data = value["data"] as? JSON {
                                                    if let faceId = data["faceId"] as? String, let driverName = data["driverName"] as? String {
                                                        strongSelf.addCamera(id: UInt64(faceId) ?? 0, userName: driverName)
                                                        return
                                                    }
                                                }
                                            }else{
...
extension UIAlertController{
    func addColorInTitleAndMessage(color:UIColor,titleFontSize:CGFloat = 18, messageFontSize:CGFloat = 13){
        let attributesTitle = [NSAttributedString.Key.foregroundColor: color, NSAttributedString.Key.font: UIFont(name: "BeVietnamPro-Bold", size: titleFontSize)]
        let attributesMessage = [NSAttributedString.Key.foregroundColor: color, NSAttributedString.Key.font: UIFont(name: "BeVietnamPro-Regular", size: messageFontSize)]
