...
@IBDesignable class MMSlidingButton: UIView{
    ...
    @IBInspectable var buttonText: String = NSLocalizedString("UNLOCK", comment: "UNLOCK") {
        didSet{
            setStyle()
        }
    }
    
    @IBInspectable var dragPointText: String = NSLocalizedString("UNLOCK", comment: "UNLOCK") {
        didSet{
            setStyle()
        }
    }
    ...
    @IBInspectable var buttonUnlockedText: String   = NSLocalizedString("UNLOCKED", comment: "UNLOCKED")
    @IBInspectable var buttonUnlockedColor: UIColor = UIColor.black
    var buttonFont                                  = UIFont(name: "BeVietnamPro-Bold", size: 14)!
    ...
    func setUpButton(){
        ...
        if !self.buttonText.isEmpty{
            ...
            self.dragPointButtonLabel               = UILabel(frame: CGRect(x: self.frame.size.width - dragPointWidth, y: 0, width: dragPointWidth, height: self.frame.size.height))
            self.dragPointButtonLabel.textAlignment = .center
            self.dragPointButtonLabel.text          = dragPointText
    ...
    @objc func panDetected(sender: UIPanGestureRecognizer){
        ...
        if translatedPoint.x > 0.5 * (frame.width - dragPointWidth) {
            dragPointButtonLabel.frame = CGRect(x: 1.5 * (frame.width - dragPointWidth) - translatedPoint.x, y: 0, width: dragPointWidth, height: frame.height)
        }
