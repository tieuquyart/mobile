...
class HNCSSDCardViewController: BaseTableViewController, CameraRelated {
    ...
    override func viewDidLoad() {
        ...
        initHeader(text: NSLocalizedString("SD Card", comment: "SD Card"), leftButton: true)
...
//        }
    ...
    override func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        let fontM = UIFont(name: "BevietnamPro-Medium", size: 16)
        let fontR = UIFont(name: "BevietnamPro-Regular", size: 16)
        
        cell.textLabel?.font = fontM
        cell.textLabel?.textColor = UIColor.color(fromHex: "#4D5966")
        cell.detailTextLabel?.font = fontR
        cell.detailTextLabel?.textColor = UIColor.color(fromHex: "#9AA7B6")
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if HNCSSDCardSection(rawValue: indexPath.section) == .formatButton && indexPath.row == 0 {
            ...
            self.alert(title: NSLocalizedString("Format SD Card", comment: "Format SD Card"), message: NSLocalizedString("all_data_will_be_erased_from_sd_card", comment: "All data will be erased from SD card.\nAre you sure?"), cancelHandler: {}, okHandler:{
                self.prepareFormat()
            })
