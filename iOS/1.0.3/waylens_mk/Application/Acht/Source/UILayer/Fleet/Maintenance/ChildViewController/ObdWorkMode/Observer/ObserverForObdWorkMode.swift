...
class ObserverForObdWorkMode: Observer {

    weak var eventResponder: ObserverForObdWorkModeEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<ObdWorkModeViewControllerState>
    ...
    init(state: Observable<ObdWorkModeViewControllerState>) {
        self.state = state
    }
