//
//  ObserverForCalibrationCalibrate.swift
//  Fleet
//
...
class ObserverForCalibrationCalibrate: Observer {

    weak var eventResponder: ObserverForCalibrationCalibrateEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<CalibrationCalibrateViewControllerState>
    ...
    init(state: Observable<CalibrationCalibrateViewControllerState>) {
        self.state = state
    }
