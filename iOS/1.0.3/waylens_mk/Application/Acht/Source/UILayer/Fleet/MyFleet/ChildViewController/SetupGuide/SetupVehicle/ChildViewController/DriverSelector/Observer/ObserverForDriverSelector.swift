//
//  ObserverForDriverSelector.swift
//  Fleet
//
...
class ObserverForDriverSelector: Observer {

    weak var eventResponder: ObserverForDriverSelectorEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<DriverSelectorViewControllerState>
    ...
    init(state: Observable<DriverSelectorViewControllerState>) {
        self.state = state
    }
