//
//  ObserverForGeoFenceRuleList.swift
//  Fleet
//
...
class ObserverForGeoFenceRuleList: Observer {

    weak var eventResponder: ObserverForGeoFenceRuleListEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<GeoFenceRuleListViewControllerState>
    ...
    init(state: Observable<GeoFenceRuleListViewControllerState>) {
        self.state = state
    }
