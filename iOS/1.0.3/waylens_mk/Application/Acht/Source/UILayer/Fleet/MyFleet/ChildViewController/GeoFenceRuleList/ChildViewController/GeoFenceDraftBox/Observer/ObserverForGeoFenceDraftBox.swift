//
//  ObserverForGeoFenceDraftBox.swift
//  Fleet
//
...
class ObserverForGeoFenceDraftBox: Observer {

    weak var eventResponder: ObserverForGeoFenceDraftBoxEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<GeoFenceDraftBoxViewControllerState>
    ...
    init(state: Observable<GeoFenceDraftBoxViewControllerState>) {
        self.state = state
    }
