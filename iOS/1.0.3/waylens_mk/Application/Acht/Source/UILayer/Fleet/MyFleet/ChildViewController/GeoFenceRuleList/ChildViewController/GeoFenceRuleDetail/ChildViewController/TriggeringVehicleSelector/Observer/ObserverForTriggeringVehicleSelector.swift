//
//  ObserverForTriggeringVehicleSelector.swift
//  Fleet
//
...
class ObserverForTriggeringVehicleSelector: Observer {

    weak var eventResponder: ObserverForTriggeringVehicleSelectorEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<TriggeringVehicleSelectorViewControllerState>
    ...
    init(state: Observable<TriggeringVehicleSelectorViewControllerState>) {
        self.state = state
    }
