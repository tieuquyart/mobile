...
class ObserverForInstallation: Observer {

    weak var eventResponder: ObserverForInstallationEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<InstallationViewControllerState>
    ...
    init(state: Observable<InstallationViewControllerState>) {
        self.state = state
    }
