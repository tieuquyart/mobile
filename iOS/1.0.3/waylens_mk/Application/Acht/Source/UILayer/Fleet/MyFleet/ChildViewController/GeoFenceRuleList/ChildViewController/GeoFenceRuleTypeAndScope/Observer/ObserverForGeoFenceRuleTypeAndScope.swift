//
//  ObserverForGeoFenceRuleTypeAndScope.swift
//  Fleet
//
...
class ObserverForGeoFenceRuleTypeAndScope: Observer {

    weak var eventResponder: ObserverForGeoFenceRuleTypeAndScopeEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<GeoFenceRuleTypeAndScopeViewControllerState>
    ...
    init(state: Observable<GeoFenceRuleTypeAndScopeViewControllerState>) {
        self.state = state
    }
