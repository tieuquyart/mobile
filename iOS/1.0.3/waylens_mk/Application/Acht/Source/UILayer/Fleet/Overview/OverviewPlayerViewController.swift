...
import MapKit
 class OverviewPlayerViewController<HeaderViewType>: BaseViewController, HNPlayerPanelDelegate  {
    ...
    override func viewDidLoad() {
        ...
        playerContainerView.layer.cornerRadius = 10
        playerPanel.view.layer.cornerRadius = 10
        ...
        if HeaderViewType.self ==  OverviewLiveHeaderView.self {
            print("headerViewType: \(HeaderViewType.self)")
            self.headerViewHeightConstraint.constant = 100
            self.headerViewContainerView.layoutIfNeeded()
        }else{
            print("headerViewType: \(HeaderViewType.self)")
