...
class ObserverForVehicleList: Observer {

    weak var eventResponder: ObserverForVehicleListEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<VehicleListViewControllerState>
    ...
    init(state: Observable<VehicleListViewControllerState>) {
        self.state = state
    }
