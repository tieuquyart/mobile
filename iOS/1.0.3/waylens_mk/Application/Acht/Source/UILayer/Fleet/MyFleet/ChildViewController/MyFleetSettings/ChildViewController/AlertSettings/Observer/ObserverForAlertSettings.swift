//
//  ObserverForAlertSettings.swift
//  Fleet
//
...
class ObserverForAlertSettings: Observer {

    weak var eventResponder: ObserverForAlertSettingsEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<AlertSettingsViewControllerState>
    ...
    init(state: Observable<AlertSettingsViewControllerState>) {
        self.state = state
    }
