...
class ObserverForAddNewCamera: Observer {

    weak var eventResponder: ObserverForAddNewCameraEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<AddNewCameraViewControllerState>
    ...
    init(state: Observable<AddNewCameraViewControllerState>) {
        self.state = state
    }
