//
//  ObserverForActivateCamera.swift
//  Fleet
//
...
class ObserverForActivateCamera: Observer {

    weak var eventResponder: ObserverForActivateCameraEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<ActivateCameraViewControllerState>
    ...
    init(state: Observable<ActivateCameraViewControllerState>) {
        self.state = state
    }
