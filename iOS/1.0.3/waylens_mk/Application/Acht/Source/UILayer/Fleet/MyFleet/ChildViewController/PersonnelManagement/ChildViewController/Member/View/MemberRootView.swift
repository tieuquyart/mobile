     ...
     //   let roles = memberProfile.roles
       ...
       // phoneNumberRow.detail = memberProfile.phoneNumber
        ...
        if scene == .addingNew {
            ...
            let saveButton = ButtonFactory.makeBigBottomButton(NSLocalizedString("Save", comment: "Save"), color: UIColor.black)
            saveButton.addTarget(self, action: #selector(saveButtonTapped(_:)), for: .touchUpInside)
            saveButton.setBackgroundImage(with: UIColor.lightGray, for: .disabled)
...
//                }
            ...
            if !memberProfile.isOwner {
                if UserSetting.current.userProfile?.isOwner == true && memberProfile.roles.contains(.fleetManager) && memberProfile.isVerified {
                    let setOwnerButton = ButtonFactory.makeBigBottomButton(NSLocalizedString("Set as Fleet Owner", comment: "Set as Fleet Owner"), color: UIColor.black)
                    setOwnerButton.addTarget(self, action: #selector(setOwnerButtonTapped(_:)), for: .touchUpInside)
                    addBottomItemView(setOwnerButton)
                }

                let removeButton = ButtonFactory.makeBigBottomButton(NSLocalizedString("Remove", comment: "Remove"), color: UIColor.white)
