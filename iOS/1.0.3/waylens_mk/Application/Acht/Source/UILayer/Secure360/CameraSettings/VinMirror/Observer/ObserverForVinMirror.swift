...
class ObserverForVinMirror: Observer {

    weak var eventResponder: ObserverForVinMirrorEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<VinMirrorViewControllerState>
    ...
    init(state: Observable<VinMirrorViewControllerState>) {
        self.state = state
    }
