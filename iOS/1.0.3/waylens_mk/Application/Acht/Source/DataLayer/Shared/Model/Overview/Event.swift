...
extension HNClip {
    ...
    convenience init(event: Event) {
        ...
        print("doanvt-hn", (event.startTime?.miliSecFromDate())! as Double)
        
        let clipDict: [String : Any] = [
            "momentID" : Int64(event.id!),
            "cameraSN" : event.cameraSn!,
            "mp4Url" : event.url ?? "",
            "startTime" :(event.startTime?.miliSecFromDate())! as Double,
    ...
    convenience init(notiItem: NotiItem){
        ...
        print("doanvt-hn", notiItem.eventTime?.miliSecFromDate() as Any)
...
extension String {
    func miliSecFromDate() -> TimeInterval? {
            let strTime = self
//        let timeArr = strTime.components(separatedBy:"T")
        
            let formatter = DateFormatter()
            formatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss"
        let ObjDate = formatter.date(from: String(strTime))
            return (ObjDate?.millisecondsSince1970)
    }
