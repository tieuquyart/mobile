    ...
    func formatTextLicensePlate() -> Bool {
        ...
        if let licenseText = lblShowLicensePlate.text {
            if licenseText.isEmpty  {
                self.alert(title: "Thông báo", message: "Xuất báo cáo Không có dữ liệu")
                return false
            }
        }
        return true
...
//        }
    ...
    {
        SVProgressHUD.show(withStatus: NSLocalizedString("Đang tải...", comment: "Đang tải..."))
               ...
               do {
                 ...
                 //  let startTime = self.dateRange.from.dateManager.fleetDate.dateAt(.startOfDay).date.toString(format: .isoDate)
                   ...
                   DispatchQueue.main.async {
                       ...
                       self.alert(title: "Thông báo", message: "Tải về thành công")
...
//            view.layer.borderColor = UIColor.color(fromHex: ConstantMK.borderGrayColor).cgColor
    ...
    func configBtnDownload() {
        let downLoadImg  = UIImage(named: "btn_export_n")!
        btnDownload.addLeftIcon(image: downLoadImg)
        btnDownload.tintColor = UIColor.white
        btnDownload.backgroundColor = UIColor.color(fromHex: ConstantMK.blueButton)
    }
    ...
    @IBAction func clickTokenJpushBtn(_ sender: Any) {
        self.alert(title: "Thông báo", message: "Copy thành công")
    }
...
func showToast(message : String, seconds: Double, completion: (() -> Void)? = nil){
    ...
    func toastMessage( message: String){
        ...
        messageLbl.font = UIFont(name: "BeVietnamPro-Regular", size: 12)!
