...
class DeviceViewController: BaseViewController {
    ...
    @IBOutlet weak var imgConnectWifi: UIImageView!
    
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = .white
        self.imgConnectWifi.image = UIImage(named: "icon_connectWifi")
        self.labelConectWifi.text = "Cài đặt wifi"
        ...
        setBorderView(view: viewContainerConnectCamera)
         let driverVC = DriverListController()
       let  vehicleVC = VehicleListController()
       let cameraVC = CameraListController()
        
        let viewControllers = [vehicleVC,cameraVC,driverVC]
        
       
        let pagingViewController = PagingViewController(viewControllers: viewControllers)
        pagingViewController.menuItemSize = .fixed(width: screenWidth / 3 , height: 40)
        pagingViewController.font = UIFont(name: "BeVietnamPro-Medium", size: 16)!
        pagingViewController.selectedFont = UIFont(name: "BeVietnamPro-Medium", size: 16)!
...
extension DeviceViewController : ObserverForCurrentConnectedCameraEventResponder {
    func connectedCameraDidChange(_ camera: WLCameraDevice?) {
        ...
        if let newCameraConnected = camera, newCameraConnected.productSerie != .unknown {
            self.labelConectWifi.text = "\(camera?.sn ?? "")"
            self.imgConnectWifi.image = UIImage(named: "icon_connectWifi")
            self.imgConnectWifi.tintColor = UIColor.color(fromHex: ConstantMK.greenLabel)
            ...
            self.labelConectWifi.textColor = UIColor.color(fromHex: ConstantMK.grayLabel)
            self.imgConnectWifi.tintColor = UIColor.color(fromHex: ConstantMK.grayLabel)
            self.viewContainerConnectCamera.backgroundColor = UIColor.color(fromHex: ConstantMK.grayBG)
          }
    }
}
