...
class AddDriverController: BaseViewController {
    ...
    @IBOutlet weak var viewBirthOfDate: UIView!
    @IBOutlet weak var viewPhoneNo: UIView!
    @IBOutlet weak var viewIdNo: UIView!
    @IBOutlet weak var viewLicenseNo: UIView!
    @IBOutlet weak var viewExpireDate: UIView!
    @IBOutlet weak var viewDriverCode: UIView!
    ...
    override func viewDidLoad() {
        ...
        self.borderTF([viewName,viewBirthOfDate,viewPhoneNo,viewIdNo,viewLicenseNo, btnShowView, viewExpireDate, viewDriverCode])
    }
    
    func borderTF(_ items : [UIView]){
        items.forEach { tf in
            tf.layer.cornerRadius = 8
            tf.addShadow(offset: CGSize(width: 3, height: 4))
        }
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if !isAddDrive {
            ...
            initHeader(text: "Sửa Tài Xế", leftButton: false)
            disableTf([codeDriver])
            disableV([viewDriverCode])
            ...
            initHeader(text: "Thêm Tài Xế", leftButton: false)
        }
    ...
    @IBAction func okBtn(_ sender: Any) {
        if formatText() {
            if isAddDrive {
                self.alert(title: "Thông báo", message: "Bạn chắc chắn muốn thêm user \(driverNameTf.text ?? "") vào đội xe", okHandler: {
                    self.add()
                })
            } else {
                self.alert(title: "Thông báo", message: "Bạn chắc chắn muốn sửa thông tin user \(driverNameTf.text ?? "")", okHandler:  {
                    self.change()
                })
            }
        }
 ...
 //   var formattedDate: String? = ""
    ...
    func formatText() -> Bool {
        if let nameText = driverNameTf.text {
            if nameText.isEmpty  {
                self.alert(title: "Thông báo", message: "Hãy nhập thông tin Tên tài xế")
                return false
            }
        }
        ...
        if let birthdayUser = txtBirthdayUser.text {
            if birthdayUser.isEmpty  {
                self.alert(title: "Thông báo", message: "Hãy chọn ngày sinh tài xế")
                return false
            }
        }
        ...
        if let nameText = idNumberTf.text {
            if nameText.isEmpty  {
                self.alert(title: "Thông báo", message: "Hãy nhập thông tin Số CCCD")
                return false
            }
            
            if !(nameText.count == 12) {
                self.alert(title: "Thông báo", message: "Độ dài CCCD là 12 số")
                return false
            }
        }
        ...
        if let phoneText = phoneNumberTf.text {
            if phoneText.isEmpty  {
                self.alert(title: "Thông báo", message: "Hãy nhập thông tin Số điện thoại")
                return false
            }
            
            if !phoneText.validatePhone() {
                self.alert(title: "Thông báo", message: "Lỗi định dạng Số điện thoại")
                return false
            }
        ...
        if let  driverLicenseText = driverLicenseTf.text {
            if driverLicenseText.isEmpty  {
                self.alert(title: "Thông báo", message: "Hãy nhập thông tin Giấy phép lái xe")
                return false
            }
            
            if !(driverLicenseText.count == 12) {
                self.alert(title: "Thông báo", message: "Độ dài số Giấy phép lái xe là 12 số")
                return false
            }
        }
        ...
        if let  rankText = btnShowView.infoLabel.text {
            if rankText.isEmpty  {
                self.alert(title: "Thông báo", message: "Hãy nhập thông tin Hạng")
                return false
            }
        }
        ...
        if let  timeText = valid_until_tf.text {
            if timeText.isEmpty  {
                self.alert(title: "Thông báo", message: "Hãy nhập thông tin thời hạn của Giấy phép lái xe")
                return false
            }
        }
        ...
        if let  codeText = codeDriver.text {
            if codeText.isEmpty  {
                self.alert(title: "Thông báo", message: "Hãy nhập thông tin Mã hiệu tài xế")
                return false
            }
            
            if  6 < codeText.count || codeText.count > 50 {
                self.alert(title: "Thông báo", message: "Mã hiệu tài xế không đúng định dạng")
                return false
            }
        }
        return true
