...
class AddCameraViewController: BaseViewController {
    ...
    @IBOutlet weak var viewCameraSN : UIView!
    ...
    override func viewDidLoad() {
        ...
        borderTF([viewCameraSN,viewPwd,viewPhoneNo,viewCameraType,viewDate])
        disableTf([cameraTypeTf])
        disableV([viewCameraType])
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if !isAddCamera {
            initHeader(text: "Sửa thông tin camera", leftButton: false)
            disableTf([snTf])
            disableV([viewCameraSN])
            snTf.text = model.sn
            ...
            initHeader(text: "Thêm camera", leftButton: false)
        }
    ...
    func formatText() -> Bool {
        if let snText = snTf.text {
         //   print("snText.count",snText.count)
            if snText.isEmpty || !snText.contains("6B") || !(snText.count == 8) {
                self.alert(title: "Thông báo", message: "Camera S/N nhập sai định dạng (6BXXXXXX)")
                return false
            }
        }
        ...
        if let pwd = passwdTf.text {
            if pwd.isEmpty{
                self.alert(title: "Thông báo", message: "Vui lòng nhập mật khẩu")
                return false
            }
            
            if pwd.count != 8 {
                self.alert(title: "Thông báo", message: "Mật khẩu gồm 8 chữ số")
                return false
            }
        }
