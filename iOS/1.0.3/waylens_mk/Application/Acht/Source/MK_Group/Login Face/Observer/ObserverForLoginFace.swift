...
class ObserverForLoginFace: Observer {

    weak var eventResponder: ObserverForLoginFaceViewEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<LoginFaceViewControllerState>
    ...
    init(state: Observable<LoginFaceViewControllerState>) {
        self.state = state
    }
