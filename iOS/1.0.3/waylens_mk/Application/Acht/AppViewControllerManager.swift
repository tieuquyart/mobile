...
class AppViewControllerManager {
    ...
    static func createTabBarController() -> TabBarController {
        ...
        overviewVC.tabBarItem = UITabBarItem(title: NSLocalizedString("Overview", comment: "Overview"), image: UIImage(named: "icon_overview"), tag: 0)
        overviewVC.tabBarItem.setTitleTextAttributes(att, for: .normal)
        let dashboardVC  = DashboardMKViewcontroller(nibName: "DashboardMKViewcontroller", bundle: nil).embedInNavigationController()
        dashboardVC.tabBarItem = UITabBarItem(title: NSLocalizedString("Dashboard", comment: "Dashboard"), image: UIImage(named: "icon_report"),tag: 2)
        
        let deviceVC = DeviceViewController(nibName: "DeviceViewController", bundle: nil).embedInNavigationController()
        deviceVC.tabBarItem = UITabBarItem(title: NSLocalizedString("Manage", comment: "Manage"), image: UIImage(named: "icon_manage"), tag: 1)
        
        let accountVC = AccountVC(nibName: "AccountVC", bundle: nil).embedInNavigationController()
        accountVC.tabBarItem = UITabBarItem(title: NSLocalizedString("My Fleet", comment: "My Fleet"), image: UIImage(named: "icon_account"), tag: 3)
        
        viewControllers = [overviewVC, deviceVC, dashboardVC, accountVC]
        ...
        tabBarController.setFontItembar()
...
extension UIViewController {
    ...
    func alert(title: String? = nil, message: String) {
        let attTitle = [NSAttributedString.Key.font: UIFont(name: "BeVietnamPro-Bold", size: 16)]
        let attMsg = [NSAttributedString.Key.font: UIFont(name: "BeVietnamPro-Regular", size: 14)]
        let attstringTitle = NSAttributedString(string: title ?? "", attributes: attTitle as [NSAttributedString.Key : Any])
        let attstringMsg = NSAttributedString(string: message, attributes: attMsg as [NSAttributedString.Key : Any])
        
        
        let alertVC = UIAlertController(title: "", message: "", preferredStyle: .alert)
        alertVC.setValue(attstringTitle, forKey: "attributedTitle")
        alertVC.setValue(attstringMsg, forKey: "attributedMessage")
    ...
    func getAction(title: String, style: UIAlertAction.Style) -> UIAlertAction {
        let action = UIAlertAction(title: "", style: style)
        let attTitle = [NSAttributedString.Key.font: UIFont(name: "BeVietnamPro-Bold", size: 14)]
        let attstringTitle = NSAttributedString(string: title, attributes: attTitle as [NSAttributedString.Key : Any])
        action.setValue(attstringTitle, forKey: "attributedTitle")
...
//            }
    ...
    func presentCleanCacheController() {
        ...
        let title = NSLocalizedString("Clear Cache", comment: "Clear Cache")
        let msg = NSLocalizedString("clear_cache_message", comment: "total cache size: calculating...")
        let attTitle = [NSAttributedString.Key.font: UIFont(name: "BeVietnamPro-Bold", size: 16)]
        let attstringTitle = NSAttributedString(string: title, attributes: attTitle as [NSAttributedString.Key : Any])
        let attMsg = [NSAttributedString.Key.font: UIFont(name: "BeVietnamPro-Regular", size: 14)]
        let attstringMsg = NSAttributedString(string: msg, attributes: attMsg as [NSAttributedString.Key : Any])
        alert.setValue(attstringMsg, forKey: "attributedMessage")
        
        alert.setValue(attstringTitle, forKey: "attributedTitle")
...
//        alert.message = String(format: NSLocalizedString("Total cache size: %@", comment: "Total cache size: %@"), size)
        let msg2 = String(format: NSLocalizedString("Total cache size: %@", comment: "Total cache size: %@"), size)
        let attstringMsg2 = NSAttributedString(string: msg2, attributes: attMsg as [NSAttributedString.Key : Any])
        alert.setValue(attstringMsg2, forKey: "attributedMessage")
    ...
    func presentLogOutConfirmation() {
        ...
        let title = NSLocalizedString("Log Out?", comment: "Log Out?")
        let msg = "Bạn có chắc chắn muốn đăng xuất không?"
        let attTitle = [NSAttributedString.Key.font: UIFont(name: "BeVietnamPro-Bold", size: 16)]
        let attMsg = [NSAttributedString.Key.font: UIFont(name: "BeVietnamPro-Regular", size: 14)]
        let attstringTitle = NSAttributedString(string: title, attributes: attTitle as [NSAttributedString.Key : Any])
        let attstringMsg = NSAttributedString(string: msg, attributes: attMsg as [NSAttributedString.Key : Any])
    ...
    func presentRemoveAccountConfirmation() {
        let alert = UIAlertController.init(title: "", message: "", preferredStyle: .alert)
        let title = "Xóa tài khoản"
        let msg = "Bạn có chắc chắn muốn xóa tài khoản này không?"
        let attTitle = [NSAttributedString.Key.font: UIFont(name: "BeVietnamPro-Bold", size: 16)]
        let attMsg = [NSAttributedString.Key.font: UIFont(name: "BeVietnamPro-Regular", size: 14)]
        let attstringTitle = NSAttributedString(string: title, attributes: attTitle as [NSAttributedString.Key : Any])
        let attstringMsg = NSAttributedString(string: msg, attributes: attMsg as [NSAttributedString.Key : Any])
        ...
        alert.addAction(UIAlertAction.init(title: "Đồng ý", style: .destructive, handler: { (UIAlertAction) in
            SessionService.shared.removeAccount(idAccount: (UserSetting.current.userProfile?.id)!, completion: { [weak self] result in
                if let strongSelf = self {
                    switch result {
                    case .success(let value):
                        ConstantMK.parseJson(dict: value) { success, msg, code in
                            if success {
                                appDelegate.window?.rootViewController?.toastMessage(message: "Xoá tài khoản thành công")
                                AccountControlManager.shared.keyChainMgr.onLogOut()
                                AppViewControllerManager.gotoRemoveAccount()
                            } else {
                                strongSelf.toastMessage(message: "Xoá tài khoản không thành công")
                            }
                        }
                        break
                    case .failure:
