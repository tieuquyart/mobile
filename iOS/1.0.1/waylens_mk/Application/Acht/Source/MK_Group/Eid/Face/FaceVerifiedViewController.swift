...
class FaceVerifiedViewController: BaseViewController , NetworkCheckObserver {
  ...
  func capture() {
    if self.isInternet {
      self.viewLive.enrollTask { err, string in
        print("err" , err)
        print("string" , string)
      }
      self.setTimer()
...
extension FaceVerifiedViewController :  FaceViewDelegate {
  ...
  func getTemplateSuccess() {
    ...
    self.passportReader.readIdInfo(nfcPopupTitle: "Giữ iPhone của bạn gần CCCD có hỗ trợ NFC.")
    self.isLoading = true
  }
...
extension  FaceVerifiedViewController : UpdateReaderSessionMessageDelegate {
  ...
  func showMessage(alertMessage: NFCViewDisplayMessage) {
    print("value alertMessage",alertMessage.rawValue)
    switch alertMessage.rawValue {
    case 5:
      passportReader.showReaderMessage("Đã đọc thẻ CCCD thành công".localizeMk())
      break
    default:
      break
    }
  ...
  func getInfoPassPort(_ value : NFCPassportModel) {
    self.passportInfo  = value
  }
