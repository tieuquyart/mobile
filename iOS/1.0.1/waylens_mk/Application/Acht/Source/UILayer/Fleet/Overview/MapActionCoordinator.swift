...
class MapActionCoordinator {
    ...
    private func handleSurface() {
        ...
        case .tip:
            driverListVC.headerView.setImageButton(val: false)
            floatingPanelController.move(to: .full, animated: true)
        default:
            driverListVC.headerView.setImageButton(val: true)
    ...
    func presentDriverList(animated: Bool = true) {
        ...
        driverListVC.toggleSortBarHandler = { [weak self]  in
            self?.handleSurface()
        }
    ...
    func updateNavigationBar() {
        ...
        if let lastSubPanelTitle = floatingPanelController.lastSubPanel?.title {
            overviewViewController?.navigationItem.title = lastSubPanelTitle
        }
    }
