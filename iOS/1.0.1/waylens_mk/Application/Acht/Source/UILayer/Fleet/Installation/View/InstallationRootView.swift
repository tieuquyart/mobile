...
class InstallationRootView: UIView {
    ...
    private lazy var button: UIButton = { [weak self] in
        let button = ButtonFactory.makeBigBottomButton(NSLocalizedString("Start Installation Test", comment: "Start Installation Test"), color: UIColor.semanticColor(.tint(.primary)))
...
extension InstallationRootView: Themed {

    func applyTheme() {
        textView.textColor = UIColor.semanticColor(.label(.secondary))
        secondaryButton.setTitleColor(UIColor.semanticColor(.tint(.primary)), for: .normal)
    }
