...
public class NotificationListDataSource: NSObject, CardFlowViewDataSource {
    ...
    public func headerViewForCard(at index: Int, in cardFlowView: CardFlowView) -> UIView? {
        ...
        let header = UILabel()
        header.attributedText = NSAttributedString(string: item.receiveTime?.toStringUsingInMessageCell() ?? "", font: UIFont.systemFont(ofSize: 14.0), textColor: UIColor.semanticColor(.label(.primary)), indent: 20.0)
