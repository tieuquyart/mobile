...
class ObserverForGeoFenceList: Observer {

    weak var eventResponder: ObserverForGeoFenceListEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<GeoFenceListViewControllerState>
    ...
    init(state: Observable<GeoFenceListViewControllerState>) {
        self.state = state
    }
