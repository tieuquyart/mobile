...
class ObserverForAdasConfig: Observer {

    weak var eventResponder: ObserverForAdasConfigEventResponder? {
        willSet {
            if newValue == nil {
                stopObserving()
            }
    ...
    let state: Observable<AdasConfigViewControllerState>
    ...
    init(state: Observable<AdasConfigViewControllerState>) {
        self.state = state
    }
