...
class HNCSFirmwareViewController: BaseViewController, CameraRelated{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if !isExternal {
            title = NSLocalizedString("Firmware", comment: "Firmware")
        } else {
            title = NSLocalizedString("External Firmware", comment: "External Firmware")
        }
        refreshUI()
    }
        ...
        if !isLoaded && !isExternal {
            ...
            if (string?.lowercased().hasPrefix("http") == true &&
                ...
                if (WLFirmwareUpgradeManager.shared().firmwareInfo(forModel: "external")?.downloadUrl() == string &&
                    ...
                    let alert = UIAlertController.init(title: NSLocalizedString("Download firmware from this link?",
                                                                                comment: "Download firmware from this link?"),
                                                       message: string,
                                                       preferredStyle: .alert)
                    alert.addAction(UIAlertAction.init(title: NSLocalizedString("Yes", comment: "Yes"),
                                                       style: .destructive,
                                                       handler: { (action) in
                                                        self.openExternal2(string!)
                                                        WLFirmwareUpgradeManager.shared().downloadExternalFirmware(fromUrl: string!)
                    }))
                    alert.addAction(UIAlertAction.init(title: NSLocalizedString("Cancel", comment: "Cancel"),
                                                       style: .cancel,
                                                       handler: { (action) in
                                                        //
                    }))
